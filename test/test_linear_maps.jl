...
@testset "test_linear_maps" begin
    ...
    @testset "with_jacobian" begin
        ...
        for OP in [LinearMap, FunctionMap]
            ...
            # no structure declared, the is* flags derive as false:
            @test !issymmetric(J) && !ishermitian(J) && !isposdef(J)
        end

        # LinearMaps has no representation for a forward-less operator:
        @test_throws ArgumentError with_jacobian(f, x, LinearMap, ADSelector(NoAutoDiff(), ForwardDiff))
    end
end
