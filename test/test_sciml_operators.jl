...
@testset "test_sciml_operators" begin
    ...
    @testset "with_jacobian" begin
        ...
        # a missing reverse mode converts to SciML's native
        # missing-adjoint form; a missing forward mode has no SciML
        # representation:
        fwd_only = with_jacobian(f, x, SciMLOperators.FunctionOperator, ADSelector(ForwardDiff, NoAutoDiff()))[2]
        z_p = rand(Float32, 5)
        @test fwd_only * z_p ≈ J_ref * z_p
        @test !SciMLOperators.has_adjoint(fwd_only)
        @test_throws ArgumentError with_jacobian(f, x, SciMLOperators.FunctionOperator, ADSelector(NoAutoDiff(), ForwardDiff))
    end
end
