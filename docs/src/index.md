...
For Jacobians, this package provides the function [`with_jacobian`](@ref). It can return an explicit Jacobian (`OP = AbstractMatrix`) or an implicit Jacobian operator: an [`ADJacobian`](@ref AutoDiffOperators.ADJacobian) for `OP = MatrixShapedOperator` (from [MatrixShapedOperators](https://github.com/oschulz/MatrixShapedOperators.jl)), a `MulFuncOperator`, a [`LinearMap`/`FunctionMap`](https://github.com/JuliaLinearAlgebra/LinearMaps.jl) or a [`SciMLOperators.FunctionOperator`](https://github.com/SciML/SciMLOperators.jl). Lower-level functions [`with_jvp`](@ref), [`jvp_func`](@ref), [`with_vjp_func`](@ref) are provided as well.
