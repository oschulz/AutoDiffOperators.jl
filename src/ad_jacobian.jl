...
function with_jacobian(
    ...
    # Own the linearization point: `with_floatlike_contents` passes
    # already-float arrays through, but backends differ in whether
    # their same-point preparations re-read the point or capture it,
    # so a caller mutating `x` afterwards must not reach the snapshot:
    float_x === x && (float_x = copy(x))
